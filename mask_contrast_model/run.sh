...
#     do
#         asset_name="${dnn_model}_${mask}_mask_${mask}_contrast_model-MASK_OUTPUT_CORRECTED-TRAINED_ON_SERVER"
#         model_name="${dnn_model}_${mask}_mask_${mask}_contrast"
#         seed_value=11
...
#         screen -S ${screen_name} -d -m bash config.sh $cuda_device_no $asset_name $model_name $seed_value $dataset_name $dataset_path $fine_tune_embedding_model $optimizer $learning_rate $mini_batch_size $train_epochs $dropout $lime_no_of_samples $hidden_units_classifier $hidden_units_mask_embedder $hidden_units_contrast_embedder $generate_explanation_for_one_instance
...
do  
    ...
    do
        ...
        # sbatch --job-name=$model_name --partition=gpu --gres=gpu:a100:1 --mem=50G --nodelist=legolas --time=UNLIMITED --output=./log_$run/%x.out config.sh $asset_name $model_name $seed_value $dataset_name $dataset_path $fine_tune_embedding_model $optimizer $learning_rate $mini_batch_size $train_epochs $dropout $lime_no_of_samples $hidden_units_classifier $hidden_units_mask_embedder $hidden_units_contrast_embedder $generate_explanation_for_one_instance

        # screen_name=$model_name
        # screen -S ${screen_name} -d -m bash config.sh $asset_name $model_name $seed_value $dataset_name $dataset_path $fine_tune_embedding_model $optimizer $learning_rate $mini_batch_size $train_epochs $dropout $lime_no_of_samples $hidden_units_classifier $hidden_units_mask_embedder $hidden_units_contrast_embedder $generate_explanation_for_one_instance $log_folder
        # sleep 5 #so that the process starts (detects the gpu with highest mem and starts executing on it) before another process

        sbatch --job-name $asset_name config.sh $asset_name $model_name $seed_value $dataset_name $dataset_path $fine_tune_embedding_model $optimizer $learning_rate $mini_batch_size $train_epochs $dropout $lime_no_of_samples $hidden_units_classifier $hidden_units_mask_embedder $hidden_units_contrast_embedder $generate_explanation_for_one_instance
...
# # sshpass -p "#Deakin2630" rsync -avr --relative mask_contrast_model/ --exclude assets/ --exclude datasets/SST2_sentences/ guptashas@pippin.it.deakin.edu.au:/home/guptashas/PhD_experiments/CDR-mechanism/
