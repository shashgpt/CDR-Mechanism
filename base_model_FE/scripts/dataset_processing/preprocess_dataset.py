...
class Preprocess_dataset(object):
    ...
    def create_rule_masks_for_feature_extraction(self, dataset):
        """
        create rule masks for each sentence in the dataset as per the feature extraction method
        """
        rule_label_masks = []
    ...
    #     """
    #     extract the rule conjunct part for each rule sentence
    #     replace it with original sentence
    #     """
    #     for index, rule_label in enumerate(dataset["rule_label"]):
    #         if rule_label == 1:
    #             tokenized_sentence = dataset["sentence"][index].split()
    #             word_index = tokenized_sentence.index('but')
    #             tokenized_sentence = tokenized_sentence[word_index+1:]
    #             joined_sentence = ' '.join(tokenized_sentence)
    #             dataset["sentence"][index] = joined_sentence
    #         elif rule_label == 2:
    #             tokenized_sentence = dataset["sentence"][index].split()
    #             word_index = tokenized_sentence.index('yet')
    #             tokenized_sentence = tokenized_sentence[word_index+1:]
    #             joined_sentence = ' '.join(tokenized_sentence)
    #             dataset["sentence"][index] = joined_sentence
    #         elif rule_label == 3:
    #             tokenized_sentence = dataset["sentence"][index].split()
    #             word_index = tokenized_sentence.index('though')
    #             tokenized_sentence = tokenized_sentence[:word_index]
    #             joined_sentence = ' '.join(tokenized_sentence)
    #             dataset["sentence"][index] = joined_sentence
    #         elif rule_label == 4:
    #             tokenized_sentence = dataset["sentence"][index].split()
    #             word_index = tokenized_sentence.index('while')
    #             tokenized_sentence = tokenized_sentence[:word_index]
    #             joined_sentence = ' '.join(tokenized_sentence)
    #             dataset["sentence"][index] = joined_sentence
    #     return dataset
    ...
    def preprocess_covid_tweets(self, dataset):
        ...
        dataset = self.create_rule_masks_for_feature_extraction(dataset)

        # # Feature extraction (VECTORIZE)
        # print("\nFeature Extraction")
        # dataset = self.feature_extraction(dataset)
