...
if __name__=='__main__':
    ...
    # Create model
    print("\nBuilding model")
    if config["model_name"] == "rnn_FE":
        model = rnn_fe(config, word_vectors)
    elif config["model_name"] == "birnn_FE":
        model = birnn_fe(config, word_vectors)
    elif config["model_name"] == "gru_FE":
        model = gru_fe(config, word_vectors)
    elif config["model_name"] == "bigru_FE":
        model = bigru_fe(config, word_vectors)
    elif config["model_name"] == "lstm_FE":
        model = lstm_fe(config, word_vectors)
    elif config["model_name"] == "bilstm_FE":
        model = bilstm_fe(config, word_vectors)
    ...
    if config["generate_explanation"] == True:
        print("\nLIME explanations")
        Lime_explanations(config, word_index).create_lime_explanations(model)
